...
public class YBAlertController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public init() {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(YBAlertController.dismiss))
        tapGesture.numberOfTapsRequired = 1
        tapGesture.delegate = self
        self.view.addGestureRecognizer(tapGesture)
        
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(YBAlertController.changedOrientation(_:)), name: UIDeviceOrientationDidChangeNotification, object: nil)
    }
    ...
    private func initContainerView() {
        ...
        if let cancelTitle = cancelButtonTitle where cancelTitle != "" {
            ...
            cancelButton.addTarget(self, action: #selector(YBAlertController.dismiss), forControlEvents: .TouchUpInside)
    ...
    public func addButton(icon:UIImage?, title:String, action:()->Void) {
        ...
        button.addTarget(self, action: #selector(YBAlertController.buttonTapped(_:)), forControlEvents: .TouchUpInside)
